...
if st.button("Generate Report") and address and sqft > 0 and market_rent > 0:
    ...
    elif rental_estimate is not None:
        annual_turnover = rental_estimate / DEFAULT_OCR
    else:
        annual_turnover = (sqft * market_rent) / DEFAULT_OCR
    ...
    st.write(f"**Annual Turnover:** ${annual_turnover:,.2f}")
    st.write(f"**Annual Gross Profit:** ${gross_profit_calc:,.2f}")
    st.write(f"**Rental Estimate:** ${rental_estimate:,.2f}")

    # --- Generate Word Report ---
    doc = Document("Insurance Template.docx")

    # Replace question answers in blue
    for paragraph in doc.paragraphs:
        if "multi- tenanted" in paragraph.text.lower():
            paragraph.add_run(f" {multi_tenanted}").font.color.rgb = RGBColor(0,0,255)
        if "approximate age" in paragraph.text.lower():
            paragraph.add_run(f" {building_age}").font.color.rgb = RGBColor(0,0,255)
        if "total number of floors" in paragraph.text.lower():
            paragraph.add_run(f" {num_floors}").font.color.rgb = RGBColor(0,0,255)
        if "number of employees" in paragraph.text.lower():
            paragraph.add_run(f" {fte}").font.color.rgb = RGBColor(0,0,255)
        if "all values are in" in paragraph.text.lower():
            paragraph.text = f"Please see the answers in blue below. All values are in {currency} and sq ft."

    # Fill in table placeholders $
        ...
        for row in table.rows:
            for cell in row.cells:
                if "$" in cell.text and val_idx < len(table_values):
                    cell.text = f"$ {table_values[val_idx]:,.2f}"  # Format with $
                    # make value blue
    ...
    # Save to in-memory file for download
    doc_stream = BytesIO()
    doc.save(doc_stream)
    doc_stream.seek(0)

    st.download_button("📄 Download Word Report", data=doc_stream, file_name="Insurance_Report.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
